...
var aws = require('aws-sdk'),
    ...
    impl = {
        ...
        buildCloudWatchParams: function(namespace, latencies) {
            var cloudWatchParams = {
                    Namespace: namespace,
                    MetricData: []
                };
            latencies.forEach(function(latency) {
                cloudWatchParams.MetricData.push({
                    MetricName: 'ResultLatency',
                    Dimensions: [],
                    Timestamp: (new Date(latency[constants.TIMESTAMP])).toISOString(),
                    Value: latency[constants.LATENCY] / 1000000,
                    Unit: 'Milliseconds'
                });
            });
...
function CloudWatchPlugin(scriptConfig, eventEmitter) {
    ...
    eventEmitter.on('stats', function(statsObject) {
        var report = statsObject.report();
        if (!report.latencies.length) {
            return;
        }
        var params = impl.buildCloudWatchParams(self.config[constants.PLUGIN_PARAM_NAMESPACE], report.latencies);
        cloudWatch.putMetricData(params, function (err) {
            if (err) {
                console.error('Error reporting metrics to CloudWatch via putMetricData:', err);
            }
        });
